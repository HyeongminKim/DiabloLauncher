...
def LaunchGameAgent():
    ...
    if gameExecuted:
        ...
        if filteredGame == "Diablo":
            d2rLadder = "래더 5"
            d3Season = "시즌 30 | 지옥의 군주"
            d4Season = "시즌 3 | 피조물의 시즌"
...
def init():
    ...
    def AboutThisApp(*args):
        ...
        about.geometry(f"410x310+{int(root.winfo_x() + root.winfo_reqwidth() / 2 - 410 / 2)}+{int(root.winfo_y() + root.winfo_reqheight() / 2 - 310 / 2)}")
        about.deiconify()
